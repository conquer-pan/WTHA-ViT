...
from mmdet import load_checkpoint
...
class RelPosAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.table_len = self.relative_position_bias_table.size()[0]
    ...
    def forward(self, x, H, W, pos_ids, mask=None):
        """ Forward function.
        ...
        nH1 = self.num_heads
        pre_h = 2 * self.window_size[0] - 1
        pre_w = 2 * self.window_size[1] - 1
        new_h = 2 * H - 1
        new_w = 2 * W - 1
        table_pretrained_resized = F.interpolate(
            self.relative_position_bias_table.permute(1, 0).view(1, nH1, pre_h, pre_w),
            size=(new_h, new_w), mode='bicubic')
        table_pretrained_resized = table_pretrained_resized.reshape(nH1, new_h * new_w).permute(1, 0)
        relative_position_bias = table_pretrained_resized[pos_ids.view(-1)].view(H * W, H * W, -1)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, input_resolution, window_size=7, shift_size=0,
        ...
        self.attn = RelPosAttention(
            dim, window_size=self.input_resolution, num_heads=num_heads,
    ...
    def forward(self, x):
        x = x + self.drop_path(self.attn(self.norm1(x), self.H, self.W, self.pos_ids))
...
class LITLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        """
        if self.has_msa:
            pos_ids = self.get_relative_pos_index(H, W)
...
class LIT(nn.Module):
    def __init__(self,
                 ...
                 use_checkpoint=False,
                 has_msa=[0, 0, 1, 1],
                 init_cfg=dict(type='Pretrained', checkpoint='/home/omeneis/pjh/mmdetection-2.26.0/work_DOTA1-1024/retina_r50/epoch_12.pth')):
        super().__init__()
        # self.fp16_enabled = True
        ...
        input_resolution = [800 // patch_size, 800 // patch_size]
